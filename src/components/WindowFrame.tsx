...
const WindowFrame = memo(function WindowFrame({ window: win, children }: WindowFrameProps) {
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ position: 'absolute', top: 0, left: RESIZE_HANDLE, right: RESIZE_HANDLE, height: RESIZE_HANDLE, cursor: 'n-resize', pointerEvents: isMaximized ? 'none' : 'auto' }} />
        <div style={{ position: 'absolute', bottom: 0, left: RESIZE_HANDLE, right: RESIZE_HANDLE, height: RESIZE_HANDLE, cursor: 's-resize', pointerEvents: isMaximized ? 'none' : 'auto' }} />
        <div style={{ position: 'absolute', left: 0, top: RESIZE_HANDLE, bottom: RESIZE_HANDLE, width: RESIZE_HANDLE, cursor: 'w-resize', pointerEvents: isMaximized ? 'none' : 'auto' }} />
        <div style={{ position: 'absolute', right: 0, top: RESIZE_HANDLE, bottom: RESIZE_HANDLE, width: RESIZE_HANDLE, cursor: 'e-resize', pointerEvents: isMaximized ? 'none' : 'auto' }} />
        <div style={{ position: 'absolute', top: 0, left: 0, width: RESIZE_HANDLE * 2, height: RESIZE_HANDLE * 2, cursor: 'nw-resize', pointerEvents: isMaximized ? 'none' : 'auto' }} />
        <div style={{ position: 'absolute', top: 0, right: 0, width: RESIZE_HANDLE * 2, height: RESIZE_HANDLE * 2, cursor: 'ne-resize', pointerEvents: isMaximized ? 'none' : 'auto' }} />
        <div style={{ position: 'absolute', bottom: 0, left: 0, width: RESIZE_HANDLE * 2, height: RESIZE_HANDLE * 2, cursor: 'sw-resize', pointerEvents: isMaximized ? 'none' : 'auto' }} />
        <div style={{ position: 'absolute', bottom: 0, right: 0, width: RESIZE_HANDLE * 2, height: RESIZE_HANDLE * 2, cursor: 'se-resize', pointerEvents: isMaximized ? 'none' : 'auto' }} />
      </div>
      ...
      >
        <ErrorBoundary
          fallback={(error, reset) => (
            <div className="flex flex-col items-center justify-center h-full p-6 text-center select-none bg-[var(--bg-window)] text-[var(--text-primary)]">
              <div className="w-16 h-16 rounded-full bg-red-500/10 flex items-center justify-center mb-4">
                <Icons.AlertTriangle size={32} className="text-red-500" />
              </div>
              <h3 className="text-base font-semibold mb-2">Application Crash</h3>
              <p className="text-xs text-[var(--text-secondary)] mb-6 max-w-[240px] break-words line-clamp-3">
                {error?.message || 'An unexpected error occurred.'}
              </p>
              <button
                onClick={reset}
                className="px-4 py-2 bg-red-600 hover:bg-red-700 active:bg-red-800 text-white rounded-lg text-xs font-medium transition-colors cursor-pointer"
              >
                Restart Application
              </button>
            </div>
