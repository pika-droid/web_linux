// ============================================================
// Virtual File System — In-Memory with localStorage Persistence & React Context
// ============================================================
...
// ---- Context Definitions ----
export interface VFSContextType {
  fs: FileSystemState;
...
export const VFSProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const createFile = useCallback(
    (parentId: string, name: string, content = '') => {
      if (!isValidName(name)) {
        throw new Error('Invalid name: path traversal or invalid characters detected');
      }
      const id = generateId();
...
export const useFileSystem = (): VFSContextType => {
  const context = useContext(VFSContext);
  if (!context) {
    throw new Error('useFileSystem must be used within a VFSProvider');
  }
  return context;
};
